...
# Simultaneous calculate the mean accuracy of each model defined by k on the validation set data

# Test models for k = 1,max_k_train = 10% of training set size

max_k = int(np.floor(y_train.shape[0] * 0.1))
accuracy_scores = []
ks = []
...
# Predict class for each example in validation set (for fixed k)

    y_pred = neigh.predict(X_CV)

    ks.append(i+1)
    accuracy_scores.append(accuracy_score(y_CV, y_pred))
...
k_max = np.argmax(accuracy_scores) + 1
...
print 'Accuracy scores based on validation', accuracy_scores

print 'Optimal model: k=', k_max, "Maximal accuracy = ", test_set_accuracy

#np.savetxt(filename,'Optimal model: k=', k_max, ' and maximal accuracy = ', test_set_accuracy)
np.savetxt(filename,zip(ks,accuracy_scores),delimiter=',')
